...
#   - 現ブランチが feature/<X> または feature/<X>/<sub-branch>... → <X>/
#     （<X> は feature/ 直下の先頭セグメントのみを採用。深い階層は無視）
...
extract_theme() {
    # "feature/foo"         → "foo"
    # "feature/foo/bar/baz" → "foo"
    local ref="$1"
    local rest="${ref#feature/}"
    echo "${rest%%/*}"
}

# 1. 現ブランチが feature/* 自身か判定
current=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "")
if [[ "$current" == feature/* ]]; then
    theme=$(extract_theme "$current")
    echo "${theme}/"
    exit 0
fi
