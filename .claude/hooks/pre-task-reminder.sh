...
cat <<'HEADER'
<system-reminder>
新規の作業指示を受けた場合、応答着手前に以下を確認すること。継続応答・clarification・軽微な確認ではこの限りでない（毎ターン繰り返しになるノイズを避けるため Claude が判断）。なお、あなたが sub-agent として呼び出されている場合は本確認をスキップする（sub-agent のモデル選定は親側で決定済みのため）。
HEADER
