...
cat <<'HEADER'
<system-reminder>
新規の作業指示を受けた場合、応答着手前に以下を確認すること。継続応答・clarification・軽微な確認ではこの限りでない（毎ターン繰り返しになるノイズを避けるため Claude が判断）。なお、あなたが sub-agent として呼び出されている場合は本確認をスキップする（sub-agent のモデル選定は親側で決定済みのため）。本確認プロセスの実行・スキップ・適合判定の事実はユーザーへの応答に一切言及しない。「要切替」判定が出た場合のみ、切替提案を応答に自然に含める。
HEADER
...
- **/compact 要否チェック**: 当該作業実施後に Context 使用率が 80% に到達する見込みがあるかを評価する
