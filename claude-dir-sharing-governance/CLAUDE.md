...
## メンテナンス指針（C-CRI で本テーマを保守する）

本テーマは `research-by-cc`（調査ワークスペース）から移管した完成版の正本。C-CRI 単独で保守できるが、次の 3 点に注意する。

1. **手順書が記述するスクリプトの「正」は `base-dev-kit-for-cc`（C-BDK）の `scripts/` にある現行版**。手順書（`reports/02.../Marketplace外資産_開発・テスト_手順書.md` §6/§7 ほか）の `publish-share` / `check-assets` / `clean-test-env` を保守するときは、**C-BDK の現行スクリプトと突き合わせる**（Sonnet などに実スクリプトとの読み合わせを委任するのが有効。v1.7 はその方法で乖離 4 件を修正した）。
2. **⚠ `reports/02.../02.Marketplace外資産編/scripts/` に置かれたスクリプトは研究当時の凍結スナップショットであり、現行版ではない**（例: `publish-share.sh` は旧仕様の「`--ref` 既定 `main`」のまま。現行は `--ref` 必須）。**照合の基準にしてはならない**。あくまで報告書作成時点の資料。正は常に C-BDK 側。
3. **本リポジトリ（C-CRI）のブランチ運用・活動フォルダ規約に従う** ―― `feature/<X>` → 活動フォルダ `<X>/`、`feature/*`→`develop`→`main` のマージ順（ルート `CLAUDE.md` 参照）。移管元 `research-by-cc` の規約（`research-for-<X>/`・単一ファイル版管理等）とは異なるので、C-CRI で作業するときは C-CRI の規約を優先する。`reports/` 内部の配置・命名は移管時のまま保持している。
...
- **核心結論（v2.0・2026-09-14 更新／クロスレビュー 0.9 版まで反映済み）**: `.claude/` 全体を単一手段で配る方法は無い。**条件付きロード（`paths:` 付き rule・サブディレクトリの `CLAUDE.md` / `skills`）は Read ツールでしか発火しない**ため出し分けの機構に使えない（実測）。したがって **`.claude/` をひとつの Git リポジトリにし、作業リポジトリの `.claude/` として clone してブランチで作業種別を切り替える（案C-1）**。`--add-dir` / `--settings` / 環境変数はいずれも不要。
  - ⚠ v1.2 までの「3 配布チャネル（層1/層2/層3）＋ `--add-dir` 補助」という整理は**v2.0 で置き換えた**。v1.2 は時点記録として保持している。**破棄したのは「4 分類で尽きる」という網羅性の主張だけ**で、層1/層2/層3 という語自体は v2.0 でも使う。
  - ⚠ **`permissions.allow` が入れ子リポジトリ構成で適用されるかは未確認（V7）**。公式は「trust は入れ子 git リポジトリを除いた git リポジトリルートを鍵にする」（BASE-D L81157）「入れ子リポジトリには対話セッションで別途 trust ダイアログが出る／`claude -p` では出ない」（L81180）と記述しており、**対話セッションでの実測が展開前に要る**。**`deny` / `ask` は trust 非依存なので影響を受けない**（L81153）。
  - ⚠ **既存配布キット 3 リポ（C-BDK / C-BDC / C-BCP）の去就は作業指示者判断**（v2.0 §7-0）。本書の推奨は「C-BDC を流用し、C-BCP の submodule 参照を解除し、publish-share / check-assets は配布ゲートとして存続させる」。

## 成果物

| 成果物 | 所在 |
|---|---|
| 結論・構成案（**確定版 v2.0 ＝ 正本**） | `reports/01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v2.0.md` |
| 結論・構成案 v1.2（**時点記録。設計判断の根拠には使わない**。冒頭にバナー掲出済み） | `reports/01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v1.2.md` |
| **settings キー × 宣言場所 × スコープ別有効性（v1.0・D-12 の回答）** | `reports/01.配布・統制方針調査/settings宣言場所・スコープ別有効性_v1.0.md` |
| **v2.0 の実測スクリプト**（入れ子クローン／条件付きロードの発火契機／D/F コンフリクト／`rebase.updateRefs`／消費側更新／**ランチャーの静かなブランチ破損**） | `reports/01.配布・統制方針調査/verification/` |
| クロスレビュー報告書（**v2.0 に対する 3観点・各0.9版**＋ v1.2 に対する 3観点・各1.0版） | `reports/01.配布・統制方針調査/レビュー/`（論理整合性／実用性＋出典照合／作業指示者＋人間読み手） |
| 層2 配布物の開発・テスト 調査結果 | `reports/02.配布物の開発・テスト/01.Plugin・Marketplace編/Plugin・Marketplace配布物の開発・テスト_調査結果.md` |
| **Plugin / Skill 開発・テスト手順書**（**開発シナリオ 2 パターン**＝派生型／公開先行型） | `reports/02.配布物の開発・テスト/01.Plugin・Marketplace編/Plugin開発・テスト_手順書.md` |
...
| **公式ドキュメント最新化レポート**（**エグゼクティブサマリ ＋ 作業指示者の判断が必要な事項 D-1〜D-10** ＋ オーケストレーション設計・文書別変更・実機検証の記録） | `reports/01.配布・統制方針調査/レビュー/公式ドキュメント最新化レポート_2026-08-20.md` |

> 各成果物の版は各ファイル末尾の変更履歴を参照（索引には版番号を持たせない。上表で版を書いているのは v1.2 と v2.0 が併存する事情による）。
...
- reports/ 配下の確定版は **単一ファイル＋ファイル内変更履歴** で版を表現する（移管元 `research-by-cc` の版管理方針を踏襲。同一内容を別ファイルで複数回残すのはレビュー報告書の 1 回目/2 回目のような場合に限る）。
...
- [x] **公式ドキュメント最新版（CLI v2.1.235 相当・2026-08-19 取込）との全面照合・最新化**（2026-08-20）。前回の全面照合（2026-06-29・v2.1.195）以降の差分が対象。**Opus/Sonnet の SubAgent を「3 観点（陳腐化・不整合・改善機会）を各エージェント内に内包させ、文書群別に分割」して 7 体**（内訳: 変更インベントリ作成＝Sonnet ／ v1.2 結論・構成案＝**Opus** ／ 02.Plugin編＝Sonnet ／ 02.Marketplace外資産編＝Sonnet ／ 03 テンプレート＝Sonnet ／ 04 インベントリ群＝Sonnet ／ 新機能起点の横断改善提案＝**Opus**）＋ 適用 3 体（Sonnet）。**うち検出 5 体は並列実行**し、変更インベントリは先行、横断改善提案は後発で起動した。**観点別に分けなかったのは、3 観点が同一の読み込みから判定できるため**（観点別に割ると同じ本文を 3 回読ませることになる）。**検出 80 件**。主な結果:
...
- [x] **Skill/plugin の開発シナリオ 2 パターンで手順書を刷新**（2026-08-20）。旧手順書は「既存リポジトリの活動の中で必要になって作り、整えて公開する」入口だけを前提にしており、「**公開前提で思いついたものを、その開発自体を作業として一気に公開まで持っていく**」入口が抜けていた。**[手順書 v2.0](reports/02.配布物の開発・テスト/01.Plugin・Marketplace編/Plugin開発・テスト_手順書.md)** を「§1 シナリオ A（派生型）／§2 シナリオ B（公開先行型）／§3 共通コア」の構成へ全面改稿し、**CLI v2.1.237 の実機計測**を [調査結果 §9](reports/02.配布物の開発・テスト/01.Plugin・Marketplace編/Plugin・Marketplace配布物の開発・テスト_調査結果.md) に新設した（10 セルのロード挙動マトリクス＋実体リポの導線検証）。主な確定事項:
  - **D-4 の答え = 「単一実体には畳める。単一の消費形態にはならない」**。`.claude/skills/<name>/` に `plugin.json` を足し（案B''）、marketplace からは **`git-subdir`** でそのサブディレクトリを直接参照すれば、**実体 1 つ・参照 2 通り**（コピーも publish も不要）に畳める。**`plugin.json` の付与は既存利用者に非破壊**（素の skill としての呼び名 `/<name>` は project でも `--add-dir` でも不変。`--add-dir` 経由では plugin にならず素の skill のままロードされる）。**ただし層1 と層2 を同じ利用者に両方入れさせてはならない** ―― plugin どうしの衝突は CLI が抑止するが**素の project skill は残る**ため、同じ内容が二重に context へ載る（公式も「移行後は原本を削除せよ」と明記）
  - **シナリオ 2 専用リポジトリの新設は不要と確定**（2026-08-29・作業指示者確認済み）。判断フローを手順書 §2-1 に置き、**Q1 チーム共通資産か → C-BDK の `.claude/` 配下／Q2 単発・独立か → C-BDK の `.claude/` の外（例 `plugins/<name>/`。payload に乗らないので層1 へ流れない）／Q3 独自のリリースサイクルを持つか → 専用リポジトリ新設**とした。**`git-subdir` により plugin の住所は後から変更できる**（marketplace エントリの `url`/`path` の書き換えだけ）ため、初期判断を重くしない。**C-BCP（配布雛型）を開発ワークスペースに流用してはならない**ことを明文化
  - **D-2（実体リポの導線検証）を実施**。作業用の bare クローンを push 先にして**実リポジトリ・GitHub には未接触**で、実 `publish-plugin.sh` → `marketplace add` → `install` を通した。結果: **C-MKT は `.claude-plugin/marketplace.json` 不在で層2 レール未開通**／**現行 skills は `plugin.json` 無しでは `publish-plugin` に乗らない**（案B'' 形状が前提）／`plugin.json` 付与後は完走。**`publish-plugin.sh` は `read -r -p` を持つため非対話ではハングする**（C-BDK 側の実装課題としてスコープ外に記録）
  - **導線検証（`marketplace add` → `install`）を手順書で必須化**。`claude plugin validate` は相対 `source` の解決失敗を検出しないため、validate だけを根拠に「配れる」と判断しない
  - 反映先: 手順書 v2.0 ／ 調査結果 v1.5（§9 新設・出典 S27〜S31）／ v1.2 §推奨（実測による確定を追記）／ 配布計画（§3-1・§4 #1/#2・§5）／ インベントリ §0（C-MKT・開発場所の注記）

- [x] **v2.0（案C-1）への全面書き起こしと 3 観点クロスレビュー（0.9 版）**（2026-09-14）。**条件付きロードの発火契機が Read ツールに限られる**という実測（Bash `cat` / Grep / Edit 単独では不発。Edit は Read 先行を強制されるため単独では成立しない）を起点に、v1.2 の前提 2 点を破棄して全面書き起こし。推奨を **案C-1**（`.claude/` をひとつの Git リポジトリにし、作業リポジトリの `.claude/` として clone。作業種別はブランチ）へ変更し、出典を **BASE-D 単一基準**（v2.1.269・96,349 行・41 出典）へ統一した。レビューは**論理整合性／実用性＋出典照合／作業指示者＋人間読み手**の 3 観点で Round 1〜3 を回し、**CRITICAL 8 / IMPORTANT 15 / SUGGESTION 13 / POSITIVE 13**（取り下げ 0・重要度変更 0）。全件を本体へ反映済み。主な結果:
  - **ランチャーが「静かなブランチ破損」を起こす状態だった**（CRITICAL）。`git switch` が exit 128 で失敗した後に `reset --hard` が成功すると、**ブランチ名は保たれたまま中身が別ブランチで上書きされる**。**推奨していた「statusline に `.claude/` のブランチを表示する」取り違え防止策が偽陰性になる**。実測で再現を確認し（`verification/switch-fail.sh`）、全 git 呼び出しに終了コード検査を入れた版へ差し替えた（fetch 失敗＝警告して続行／switch・reset 失敗＝中断／reset 前にブランチ名一致を確認）
  - **入れ子 git リポジトリと workspace trust の関係が未検証だった**（CRITICAL）。公式は trust の単位を「**入れ子になった git リポジトリを除いた** git リポジトリのルート」と定義しており（BASE-D L81157）、**案C-1 の `.claude/` はまさにその形状**。`permissions.allow` は trust 依存・`deny` / `ask` は非依存（L81153）なので、**「permissions を一元配布できる」という主張の allow 側だけが未確認**。ただし L81180 に「入れ子リポジトリには対話セッションで trust ダイアログが出る／`claude -p` では `claude -p` 列に従う」とあり、**公式仕様上は「ダイアログが 2 枚出て、承認すれば適用される」が想定**。`claude -p` は trust ダイアログを出さない（L81161）ため**ヘッドレスでは検証不能**で、**V7 として 3 分の再現手順つきで作業指示者へ引き継ぐ**
  - **付録A に managed 専用でないキーが 4 件混入**（CRITICAL）。全 26 キーを settings-reference の Scope 列と 1 件ずつ再照合し、`permissions.disableBypassPermissionsMode` / `allowedHttpHookUrls` / `forceLoginMethod` / `forceLoginOrgUUID` の 4 件が **Scope = `Any file`** であることを確認。**managed 専用ではなくチームの `settings.json` でも設定できる**ため A-3 として分離した（「層3 が無いと使えない」という誤認を防ぐ）
  - **下流成果物との整合 4 件**（CRITICAL）。①03 実装テンプレートが v1.2 §推奨の実装のまま ②`--add-dir` 例外ロード表の正本が v1.2 と v2.0 で二重化し下流 4 文書が旧側を指したまま ③既存配布キット 3 リポの去就が未記載 ④既存ランチャーと同名ファイルが衝突し**秘匿値の受け皿が失われて共有 `settings.json` へ流れる退行リスク**。①③④は §7-0 / §7-5 / §5-6 を新設して対応し、②は下流 4 文書の正本参照を v2.0 側へ張り替えた（v1.2 側にはバナー掲出）
  - **自己訂正**: v1.2 を「凍結された時点記録」と書いていたが、**同日に v1.2 へ追記したのは本作業自身**で事実と異なる。「commit ハッシュで固定された出典を持つ社内文書」という表現へ改めた
  - **02 編手順書の「方法A／方法B」の定義を取り違えていた**ことを反映時に自己検出。方法A は `<Share>` でのネイティブ起動、方法B は `--add-dir` 結合であり、**案C-1 の形状はどちらとも一致しない**。§7-2 / §7-5 を「検証用の作業リポジトリの `.claude/` として clone して起動する」へ訂正した
  - **D-12 を解決**（[`settings宣言場所・スコープ別有効性_v1.0.md`](reports/01.配布・統制方針調査/settings宣言場所・スコープ別有効性_v1.0.md)）。公式ページ再編の写像表に加え、**軸A（同じ設定を宣言できる場所と競合時の勝敗）**・**軸B（スコープ別の有効性）**を整理。**`skipDangerousModePermissionPrompt` / `useAutoModeDuringPlan` は Scope = `User, local, or managed`**（project だけが無視される。local は有効）、**正しいキーは `sandbox.network.strictAllowlist`**、**`allowManagedHooksOnly` は plugin hooks も落とす**（managed が `enabledPlugins` で force-enable したものだけが例外・L89557）といった実害のある事実を確定した
  - **残タスク**: **1.0 版（作業指示者レビュー）**。態度表明が要るのは ①既存配布キット 3 リポの去就 ②既存ランチャーを置き換えるか併存させるか ③**V7 の対話実測**。あわせて **D-13**（下流成果物の案C-1 追随）を §8-3 に起こした
