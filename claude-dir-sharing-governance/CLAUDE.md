...
- **核心結論（v2.0・2026-09-14 更新）**: `.claude/` 全体を単一手段で配る方法は無い。**条件付きロード（`paths:` 付き rule・サブディレクトリの `CLAUDE.md` / `skills`）は Read ツールでしか発火しない**ため出し分けの機構に使えない（実測）。したがって **`.claude/` をひとつの Git リポジトリにし、作業リポジトリの `.claude/` として clone してブランチで作業種別を切り替える（案C-1）**。`--add-dir` / `--settings` / 環境変数はいずれも不要。
  - ⚠ v1.2 までの「3 配布チャネル（層1/層2/層3）＋ `--add-dir` 補助」という整理は**v2.0 で置き換えた**。v1.2 は時点記録として保持している。
...
| 結論・構成案 v1.2（**時点記録。設計判断の根拠には使わない**） | `reports/01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v1.2.md` |
| クロスレビュー報告書（3観点・各1.0版・**v1.2 に対するもの**） | `reports/01.配布・統制方針調査/レビュー/`（論理整合性／実用性＋出典照合／作業指示者＋人間読み手） |
