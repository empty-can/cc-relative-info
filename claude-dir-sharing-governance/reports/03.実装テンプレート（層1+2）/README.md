...
> ## ⚠ 本雛形は v1.2 §推奨の実装であり、現行方針（v2.0・案C-1）には追随していない
>
> 2026-09-14 に方針レポートが **[v2.0](../01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v2.0.md)（案C-1）** で確定した。案C-1 は **`.claude/` をひとつの Git リポジトリにし、作業リポジトリの `.claude/` として clone してブランチで作業種別を切り替える**構成であり、本雛形が前提にしている「各作業リポジトリへ `.claude/` を commit する」形とは**配置が違う**。
>
> **読み替え規則**:
>
> - **`layer1-repo-template/.claude/` は、共有 `.claude` リポジトリの `main` の初期コミットとして使える。** 中身（`CLAUDE.md` / `rules/` / `settings.json` / `.gitignore`）はそのまま有効だが、**`.claude/` という階層は作らず、リポジトリ直下に置く**（[v2.0 §7-1](../01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v2.0.md#sec-7-1) 手順 1.）
> - **`layer2-plugin/` はそのまま有効。** 案B（plugin 併用）は案C-1 と対立しない（[v2.0 §6-2](../01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v2.0.md#sec-6-2)）
> - **`settings.json` に置く `permissions` の扱いが変わる。** 共有リポジトリから全メンバーへ配られる設定になるため、**`env` に秘匿値を書いてはならない**（[v2.0 §3-4](../01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v2.0.md#sec-3-4)）
>
> 本雛形自体の案C-1 追随は **D-13**（[v2.0 §8-3](../01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v2.0.md#sec-8-3)）として別タスクにしてある。
