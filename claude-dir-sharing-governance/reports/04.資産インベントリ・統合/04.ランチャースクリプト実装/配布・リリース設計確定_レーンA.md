...
>
> ⚠ **用語注記（2026-08-20 追加・G5-007）**: 本書が「ランチャー」と呼ぶのは `.claude/launcher/*`・`start_claude_code.{sh,ps1}` ――利用者が端末で手動起動する `claude` セッションに `<Share>` の設定を注入する起動スクリプトを指す。公式ドキュメントには別途「[corporate launcher](https://code.claude.com/docs/en/corporate-launcher)」（`CLAUDE_CODE_PROCESS_WRAPPER` / `processWrapper`）という**同じ「ランチャー」の語を使う別機構**があり、こちらは Claude Code バイナリ自身が spawn する子プロセス（background service・agent view・Remote Control worker 等）を組織のラッパー経由に強制するもの。**両者は指す対象が異なる**。詳細な関係整理は §11「公式 corporate launcher 機構との関係」を参照。
...
> ⚠ **層3 を重ねるとランチャーが起動不能になる経路がある（2026-08-20 追加・G7-003）**: `<Share>` のランチャーに `--mcp-config` を積む設計（`TEAM_OPTS` 経由）は、配布先組織が層3（managed settings）を敷いていると**「効かない」のではなく「起動時にセッションが落ちる」**。経路は 2 本 ―― (a) `disableSideloadFlags: true`（managed 専用・v2.1.193+）が `--mcp-config` を起動時に拒否、(b) `managed-mcp.json` を配備した組織では `--mcp-config` 付きセッションが `You cannot dynamically configure MCP servers when an enterprise MCP config is present` で exit する。**`--add-dir` と `--settings` はどちらの対象でもない**ため、パターン2 の主経路（`<Share>` の CLAUDE.md/rules/skills/agents）は層3 下でも生き残る ―― 壊れるのは MCP 同梱部分だけ。層3 併用が前提の配布先では、MCP はランチャーの `--mcp-config` ではなく層3（`managed-mcp.json`/`allowedMcpServers`）か層2（plugin 同梱）で配る設計に寄せること。詳細な警告表は [v1.2 付録B](../../01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v1.2.md#appendix-b) を正本とし、本書はそれと矛盾しない要約のみを置く。
...
| **5b** | ~~公開 → submodule bump → 受入検証 → README 同期~~ → **完了（2026-07-14〜15）**。RC1 でなく **RC2 を publish**（Skills 表同期で payload が変わったため RC1 では publish 不可）。`/security-review` は **W-RBC 差分と C-BDK payload の双方で HIGH/MEDIUM 0 件**。`publish-share --ref v1.0.0-RC2` で C-BDC `8755ec8` へ 16 ファイル追加・削除 0（外向き push 成功）。C-BCP submodule bump `416f588`。**受入検証**（`git clone -c core.autocrlf=true --recurse-submodules` の C-BCP）: 対照で autocrlf 実効性を担保（導入前コミット CR 46）・ルート `.sh`=0 CR/`.ps1`=CRLF+BOM・submodule payload の hooks 2 本実在・`.sh`=LF/`.ps1`=CRLF+BOM・bash `-n`/PowerShell パーサとも構文 OK・依存解決成立。**C-BDC README を実態同期**（`ad4264e`＝「初期スキャフォールド」記述の撤去・launcher 追加・`commands/` 削除。keep-list 保護のため C-BDC へ直コミット＝R2-IM-11 後段）。**配布版 `.claude/CLAUDE.md` の Skills 表は 6 件へ同期済み**（RC2 に内包） | 達成済み | B（完了） |
| **6** | ~~受入結果に応じた昇格~~ → **完了（2026-07-15）**。受入 pass。**annotated tag `v1.0.0` を `d9dbd11`（=RC2）へ付与・push**。payload は RC2 とバイト同一のため配布物は変更なし（再 publish 不要 ―― §6 注記の実測どおり `publish-share` は「変更なし」で exit 0） | 達成済み | B（完了） |
...
- **Phase 5a は C-BDK main も動かす**（`CLAUDE.md.example` の削除）。これは **tag `v1.0.0-RC1` の後**に入るため、**タグの指す木にはまだ `CLAUDE.md.example` が残っている**。ただし `CLAUDE.md.example` は `.claude/` の外＝**payload に乗らない**ので、`publish-share --ref v1.0.0-RC1` が生む配布物は正しい（C-BDC への影響はゼロ）。C-BDK 側の削除は develop 経由で main へ入れる。
- **Phase 6 で v1.0.0 を切った後に再 publish しても壊れない**（payload が RC1 と同一なら「変更なし」になるが、`publish-share` は `git diff --cached --quiet` で分岐して **「変更なし。publish 不要」と表示し exit 0** する ―― コード実測。※ 本注記は初版で「commit 失敗で `✗` を返す可能性がある」と**誤って書いていた**。スクリプトを読まずに書いた推測であり、**「文書だけが実装と違うことを言う」を自分で再演していた**ため訂正した）。したがって Phase 6 は「タグを打つ」だけでよく、再 publish の要否は任意。
- **リリース後のドキュメント整合監査（2026-07-15）**: 3リポ公開後、利用者向け文書が実態と合うかを 1 エージェントで横断チェック。確定した実ズレ 4 件を全件修正 ―― C-BCP README が実在しない `CLAUDE.local.md.sample` を挙げていた（[中]・`130dd0e`）／C-BDC 構成表に `settings.local.json.example` 欠落（[低]・`ef8d6e5`）／C-BDK ツリーにルート `.gitignore`/`LICENSE` 欠落（[低]・`bc89bea`）／submodule ピンが RC2 世代のまま → C-BDC 最新へ bump（`130dd0e`）。**修正の過程で自作の回帰**: C-BDK ツリー補完の際、`.claude/` サブツリーに既出の `.gitignore`/`.gitattributes` を二重掲載 → 同ブランチ内で除去（`f743e3a`）。**監査した後に既知ズレを残さない・修正後に必ず grep で確認する**という本案件の教訓どおり、重複は commit 直後の検証で捕捉した。
- 会話ベースだった旧5手順を本表で置換・ファイル化（R1-IM-2 の「計画をファイル化」を満たす）。
- ~~**積み残しの残り（SUGGESTION 級）**: R3-S-1 / R3-S-2 / R2-S-2~~ → **全件解消・v1.0.1 として配布（2026-07-20）**。R2-S-2＝`start_claude_code.ps1` の `Write-Error` を `[Console]::Error.WriteLine`+`exit` へ（EAP=Stop 下で `Write-Error` が終端化し `exit` に未到達だった。**対照実験で旧 `exit 3` が終了コード 1 に潰れる＝未到達を実証**）。R3-S-1＝`custom.env` の空値を明示警告してスキップ（Windows は空文字 env を保持できず bash と黙って割れていた）。R3-S-2＝`setup-environment.ps1` の `Get-Content` に `-Encoding UTF8` を明示（PS 5.1 の既定 ANSI/CP932 読みで BOM 無し UTF-8 の日本語が化ける件の**恒久策**。テンプレの「BOM 必須」注記を緩和）。**BOM 無し UTF-8 日本語値の往復をバイト一致で検証**（⚠pwsh 7 実測。5.1 固有の効果は `-Encoding UTF8` の文書化仕様に依拠＝この環境に 5.1 が無い）。develop→main（`1800601`）・**tag `v1.0.1`**・`/security-review` HIGH/MEDIUM 0 件・`publish-share --ref v1.0.1` で C-BDC `24542c1`・`start_claude_code.ps1` を C-BCP へ直コミット＋submodule bump（C-BCP `a78738b`）・受入検証（fresh clone で両シェル構文・BOM/CRLF・修正の反映を確認）。
- ~~**R3-S-2 は恒久策が設計判断**~~ → **恒久策あり（`-Encoding UTF8`）と判明し実施済み**（上記）。「テンプレ注記のみで恒久策は保留」という初版の見立ては、実際にはコード 1 行で解けた。
- **指摘 ID は巡回ごとに振り直されている**（例: 「IM-8」は Round 1 では「設計書の正本規定」、Round 2 では「clean-test-env の env 残存」）。本表では `R1-` / `R2-` / `R3-` の巡回接頭辞を付けて区別する。

> **セキュリティゲートの構成（2026-08-20 追加・G5-004／G7-012）**
> Phase 5b の合否判定は**引き続き `/security-review`（1 パス・再現性あり）を正**とし、外さない。公式マーケットプレイスの `claude-security` プラグイン（`/plugin install claude-security@claude-plugins-official`）は**publish 対象 ref の差分に対する任意の深掘り**として併用できる（`/claude-security` → Scan changes）。ただし**スキャンは非決定的**（"two scans of the same code can surface different findings"）で、同一 ref を再スキャンすると検出が変わりうる ―― 本テーマは Round 3 で「壊れた計測どうしが『修正が効いた』ように見える」計測不正を経験しており（§10）、**再現しないゲートを合否条件にするのは同じ轍**を踏む。前提は有償プラン＋dynamic workflows＋`python3` on PATH で、配布キットの利用先すべてが満たすとは限らない。レポート出力先 `CLAUDE-SECURITY-<timestamp>/` は**自前の `.gitignore` を同梱する**ため、`check-assets` の成果物クラス（現行6種）への追加は**不要**。

### 6-bis. リリースチャネル（層2 トラックの受入前配布）（2026-08-20 追加・G7-007）

層2（C-MKT）では、**同一リポの異なる ref を指す marketplace を2本**用意することで stable / latest の2チャネルを公式にサポートできる（`plugin-marketplaces` §Set up release channels）。どちらを購読させるかは **managed settings のグループ割当**で決める。本テーマの現行 tag 運用との対応:

| チャネル | 指す ref | 用途 | 現行工程との対応 |
|---|---|---|---|
| `stable` | 版 tag（`v1.0.1` 等） | 全利用者 | Phase 6（昇格後）の配布先 |
| `latest` | `develop` | 受入検証の先行実施者 | Phase 5b の受入検証を実利用者で回す段になりうる |

> ⚠ **同一 version 解決の罠**: 2 つの ref が同じ version 文字列に解決されると、Claude Code はその2つを「同一」と見なして更新をスキップする（"If two refs resolve to the same version string, Claude Code treats them as identical and skips the update."）。本テーマは `v1.0.0` と `v1.0.0-RC2` が **payload バイト同一**だった実績があり（§6 Phase 6）、チャネル運用に移行するとこれが**「latest に上げたのに誰も更新されない」**という形の故障モードとして現れる。`plugin.json` の `version` を ref ごとに必ず変えるか、`version` を省いて解決 SHA に任せること。
>
> **⚠ 層1 body トラック（C-BDK → C-BDC → C-BCP submodule）には release channels は適用できない。** これは plugin marketplace（層2）の機構であり、submodule 配布に対応物は無い。層1 側の「先行配布」は従来どおり submodule ピンを別ブランチで進める運用になる。**層2 だけがチャネル化できる**という非対称であり、リリース工程全体をチャネル化できるわけではない。
...
| `.reg` | regedit | **スコープ外**（Claude に R/W 権限を与えない） | `.env` と同格の扱い。`settings.json` の `permissions.deny` に追加する |
| `.ini` | Win32 `GetPrivateProfile*` | **規定しない・deny もしない**（2026-07-13 訂正） | 初版は `.reg` と同列に「deny する」と書いていたが**誤り**。`tox.ini` / `pytest.ini` / `mypy.ini` のように **UTF-8 が正しい `.ini` が広く存在する**ため、拡張子で一律に deny すると通常開発が壊れる。**機構による保護は無い**ので、Win32 プロファイル用途の CP932 な `.ini` を扱う時は都度バイト列で確かめる |
| `.csv` | Excel | UTF-8 + BOM | Excel は BOM を見て UTF-8 と判定する（参考。キットには現存しない） |

> **§9-1 の `.ini` 行は PR#3 のクロスレビュー（CR-4）で訂正した**。実装（`settings.json`）には `.ini` の deny が無いのに、rule / SKILL.md / README の 3 文書が「deny 済み」と断定しており、**保護があると信じさせる誤情報が全利用先へ配布される**状態だった。配布物側は修正済み。**本書にも同じ誤りが残っていたため、ここで訂正する**（Round 3 の教訓「機構は直すが、それを説明する文書を置き去りにする」が、今度は**設計確定書そのもの**で再発していた）。
...
| `Grep` | 変換しない。`bat_cp932_guard.py sync` で全作業コピーを更新してから `.claude/.bat-shadow/` を Grep する（ディスク上の原本は触らない） |
| `Bash` | 変換しない。rule で「`.bat` を `sed -i` / リダイレクトで書き換えない」と規定 |
...
- **実装前に実測する分岐【実測結果・2026-07-13】**: `updatedInput` は **`Read` では効くが `Edit` / `Write` では無視される**（差し替えたつもりで原本に UTF-8 が書き込まれる）。しかも Edit は `old_string` の照合を hook より先に行うため、日本語を `old_string` にすると「文字列が見つからない」という無関係なエラーが先に出る。よって **`Read` は影への差し替え、`Edit` / `Write` は `permissions.deny` で原本を拒否**、という**経路ごとに手段を変える**設計になった（「Read/Edit/Write を全部ヘルパーへ一本化」でも「全部 `updatedInput`」でもない）。
- **作業コピーの置き場所と拡張子にも制約があった**（いずれも実測で判明）: `*.bat` にすると自分の `deny(**/*.bat)` に巻き込まれる（**deny は allow に勝つ**）ため `*.bat.utf8.txt` に。PostToolUse の `if` は**差し替え後のパス**で評価されるため、作業コピーをプロジェクト外（`%TEMP%`）に置くと発火しない → `.claude/.bat-shadow/`（gitignore 済み）に置く。
...
## 11. 公式 corporate launcher 機構（`CLAUDE_CODE_PROCESS_WRAPPER` / `processWrapper`）との関係【2026-08-20 追加・G7-004／G5-007】

公式に「[corporate launcher](https://code.claude.com/docs/en/corporate-launcher)」という別の「ランチャー」概念ができた。**置き換えは成立しない** ―― 解いている問題が違う。

| 観点 | 本テーマの `start_claude_code.{sh,ps1}` | 公式 `CLAUDE_CODE_PROCESS_WRAPPER` |
|---|---|---|
| 目的 | `<Share>` 資産の**設定注入**（`--add-dir`/`--settings`/`--mcp-config`/env） | 組織のセキュリティ姿勢の**強制適用**（sandbox/network/資格情報） |
| 対象プロセス | ユーザーが端末で起動する1セッション | Claude Code が自バイナリから spawn する全プロセス（background service・agent view・Remote Control worker 等。**端末起動セッションは対象外**） |
| 引数の追加 | これが主目的 | **契約で禁止**（"Don't reorder, absorb, or prepend arguments. The first argument is the Claude Code binary and everything after it is its argv."） |
| Windows | 主対応（`.ps1`・CRLF+BOM。§9） | **無視される**（"the launcher contract depends on `exec`, which Windows doesn't support. A Windows machine with the variable set runs every process unwrapped and keeps working"） |
| 配布経路 | 層1（C-BCP ルート直コミット） | managed settings の `env` ブロック（層3）／`~/.claude/settings.json`。**project/local からは設定不可**（"Project and local settings can't configure the launcher. ... Claude Code ignores `CLAUDE_CODE_PROCESS_WRAPPER` in `.claude/settings.json` or `.claude/settings.local.json` with a warning"） |

**結論: 置き換えではなく直列併用。** 本テーマのランチャーが起動するのは「ユーザーが端末で起動するセッション」＝公式が明示的に非対象とする領域（"A session you start yourself in a terminal, which runs however you invoked it."）。公式機構を要求する組織では、公式ドキュメントが示す解（PATH 上に `claude` という名前のシムを置き、その中で本ランチャーを real binary 付きで呼ぶ。**managed symlink は差し替えない**）でカバーする。逆に、本ランチャー経由で起動したセッションが後から spawn するプロセスは `CLAUDE_CODE_PROCESS_WRAPPER` の対象になるため、両者は排他ではなく直列に効く。**Windows 主体の本テーマの利用先では公式機構は効かない**ため、ランチャー配布そのものを代替する計画は立てない。

- **`processWrapper` を層3統制キーの一覧（[v1.2 付録A](../../01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v1.2.md#appendix-a)）と同一視しない**: managed 専用ではなく `~/.claude/settings.json`（user スコープ）からも設定できる。project/local から無視される点だけが本テーマに関係する。
- **新規 hook イベント `DirectoryAdded` は `<Share>` の検証に使えない**: `--add-dir` 起動フラグでは発火しないため（"Claude Code doesn't fire this event when: * You pass a directory with the `--add-dir` startup flag"）。`<Share>` の鮮度検査は従来どおり `SessionStart` に置く。

---

変更履歴: 初版（2026-07-07）。Fable クロスレビュー統合の §9 レーンA を受け、CR-1 搬送方式・CR-2 前提条件・IM-8 正本・IM-9 §ディレクトリ構造・リリースフェーズ計画を確定。IM-10（root CLAUDE.md 役割）は案X（開発リポ専用に純化）で作業指示者確定（同日）。／**2026-07-12（レーンB 着手時の補正）**: 実装・実測により本書の 2 点を補正 ―― (1) §1 CR-1 の `.claude/.gitignore` は 3 行では不足（publish の `cp -R` が C-BDC の既存 `.gitignore` を上書きするため、既存4行を含む全量を持たせないと個人設定の除外が配布のたびに失われる）。(2) §5 案X の実装には「C-BDK `CLAUDE.md.example` → C-BCP `CLAUDE.md.sample` の移管」工程が必要（chore は既に root CLAUDE.md を .example 化済み・C-BCP に雛型が未整備）＝ Phase 5a に追加。Phase 0（機械的修正 IM-1〜7/11〜13・S-1〜5 ＋ CR-1）は C-BDK `feat/launcher-scripts` にて実装完了。／**2026-07-12（Round 3 レビュー反映）**: (1) **マージ方式を「squash 推奨」から「merge commit（squash 禁止）」へ訂正**（§2・§6 Phase 1・§6 注記の 3 箇所）。squash はマージ元ブランチの履歴を消し、レビュー報告書・PR コメントが SHA で参照する修正コミット群を辿れなくするため、作業指示者の確定指示により撤回した（R3-G）。(2) **Phase 2 の CR-1/CR-2 は両 PR で実装済み**のため完了扱いに変更（R3-H）。(3) **Round 2 の「マージ後・レーンB」送り 8 件を本フェーズ表へ計上**（R2-IM-8/9/11・R2-S-1 を Phase 3 / 5b へ。R3-J＝「フェーズ表に無い約束はレーンB 移管で消える」）。(4) 指摘 ID に巡回接頭辞（`R1-` / `R2-` / `R3-`）を導入（巡回ごとに ID が振り直され「IM-8」が別物を指していたため）。／**2026-07-13（マージ実施・Windows ファイル方針の確定）**: (1) **Phase 1 完了** ―― PR#1（`400c904`）→ PR#2（`9458ed3`）を merge commit でマージし、統合後 develop を実測（reports 0・両層の check-assets が exit 0/FAIL 0・payload 26 ファイル・`.sh`=LF/`.ps1`=CRLF+BOM）。(2) **§9 を新設** ―― Windows 向けファイルの保存形式を「一律 UTF-8、CP932 は `.bat` だけの例外（`.cmd` は使用禁止・`.reg`/`.ini` はスコープ外）」で確定し、`.bat` の R/W 機構を「原本は常に CP932。Claude には UTF-8 の影を見せる（読みはディスクを触らない）」で確定。(3) **§10 を新設** ―― Round 3 の改行検証で使った `grep -c $'\r'` が Git Bash で機能しておらず（全行マッチ＝総行数を返す）、報告書と PR 本文の CR 数値が無効であることを記録。結論は再測定で追認。／**2026-07-14（Phase 3・4 完了）**: (1) **Phase 3 完了** ―― PR#3 を merge commit（`aac3378`）でマージ。Fable5 × 3 レーンのクロスレビュー指摘（CRITICAL 4・IMPORTANT 14・SUGGESTION 13）は全件反映済み（`cafb9bc`）。(2) **Phase 4 完了** ―― develop → main を merge commit（`bb4e2e1`）で統合し、annotated tag **`v1.0.0-RC1`** を付与。統合後の main を実測（reports/plans 追跡 0・check-assets が両層で exit 0/FAIL 0・payload 28 ファイル）。版数は作業指示者の `v0.9.0` 案から **`v1.0.0-RC1`** へ変更（残作業が「機能」ではなく「配線と検収」であり、RC の語義に一致するため）。**Phase 6（受入結果に応じた昇格）を新設**。(3) **§9-1 の `.ini` 行を訂正** ―― 「`permissions.deny` に追加する」は誤りで、実装にも deny は無い。`tox.ini` / `pytest.ini` 等 UTF-8 が正しい `.ini` が広く存在するため deny できない（PR#3 の CR-4）。**配布物側は修正済みだったが本書に誤りが残っていた**＝ Round 3 の教訓「機構は直すが文書を置き去りにする」が設計確定書自身で再発していた。(4) **§9-2 の「実装前に実測する分岐」を実測結果で置換** ―― `updatedInput` は Read では効くが Edit/Write では無視される、影の拡張子は `*.bat` にできない（deny が allow に勝つ）、PostToolUse の `if` は差し替え後のパスで評価される（影はプロジェクト内に置く必要がある）。／**2026-08-20（横断整合性レビュー反映・v2.1.235 版(2026-08-19) 照合）**: (1) **冒頭に用語注記を追加**（G5-007） ―― 本書の「ランチャー」（`.claude/launcher/*`・`start_claude_code.{sh,ps1}`）と、公式新設ページ「corporate launcher」（`CLAUDE_CODE_PROCESS_WRAPPER`/`processWrapper`）は指す対象が異なる別物であることを明示。(2) **§1 CR-1 末尾に層3 相互作用の警告を追加**（G7-003） ―― 層3（managed settings）を配布先が敷いていると、`disableSideloadFlags: true` と `managed-mcp.json` 配備の2経路で、ランチャーの `--mcp-config` 付き起動が「無視される」のではなく**起動時にセッションが落ちる**。`--add-dir`/`--settings` は対象外という非対称を明記し、詳細は [v1.2 付録B](../../01.配布・統制方針調査/結論・構成案_ポータブルな.claude共有_v1.2.md#appendix-b)（メインセッションが同時期に同じ警告表を追加済み）を正本として参照。(3) **§6 末尾にセキュリティゲート構成の注記を追加**（G5-004／G7-012） ―― Phase 5b の合否判定は `/security-review` を正のまま維持し、新設プラグイン `claude-security` は非決定的なスキャンのため合否条件には使わず、publish 対象 ref への任意の深掘りとして位置づけた。レポート出力先が自前 `.gitignore` を同梱するため `check-assets` の成果物クラス追加は不要である旨も明記。(4) **§6 に「6-bis. リリースチャネル」を新設**（G7-007） ―― 公式の release channels パターン（同一リポの異なる ref を指す marketplace 2本）を現行 tag 運用の上位設計として整理し、「同一 version 文字列に解決されると更新がスキップされる」罠（本テーマの `v1.0.0`/`v1.0.0-RC2` payload バイト同一の実績と直結）と、層1 body トラック（submodule 配布）にはチャネル機構が無く層2 だけがチャネル化できるという非対称を明記。(5) **§11「公式 corporate launcher 機構との関係」を新設**（G7-004） ―― 対象プロセス・引数追加可否・Windows 対応・配布経路の4観点で比較表を作成し、「置き換えではなく直列併用」「Windows 主体の本テーマの利用先では公式機構は効かない」ことを原文根拠付きで結論。あわせて `processWrapper` を付録A の managed 専用キーと同一視しないこと、新設 hook `DirectoryAdded` は `--add-dir` 起動フラグでは発火せず `<Share>` 検証に使えないことを注記。
